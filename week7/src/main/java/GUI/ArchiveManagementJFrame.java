...
import network.client.FileTransferClient;
...
// 把图标设置为登录界面的图标
    ...
    private void initComponents() {
        ...
        queryButton.addActionListener(e -> {
            ...
            if (archiveId != null && !archiveId.trim().isEmpty()) {
                try {
                    Archive archive = ClientDataProcessing.searchArchive(archiveId);
                    if (archive != null) {
                        // 清空表格
        ...
        uploadButton.addActionListener(e -> {
            ...
            confirmButton.addActionListener(evt -> {
                ...
                try {
                    // 1. 将文件上传到服务器
                    FileTransferClient.uploadFile(selectedFile);

                    // 2. 添加档案信息到系统
                    boolean success = ClientDataProcessing.insertArchive(archiveId, user.getName(), LocalDateTime.now(), description, fileName);
                    if (success) {
    ...
    private void loadArchiveData() {
        try {
            ...
            // 获取所有档案
            Collection<Archive> archives = ClientDataProcessing.getAllArchives();
